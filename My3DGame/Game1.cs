...
namespace My3DGame {
    public class Game1 : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            heroFX = new MySkinFX(Content, cam, "MySkinFX"); // makes a template effect to use for animation               

            // 3D MODEL LOADING: 
            sky.Load("sky_model");
            landscape = Content.Load<Model>("landscape");            
            Content.RootDirectory = "Content/kid";
            hero[IDLE] = Content.Load<Model>("kid_idle");
            hero[WALK] = Content.Load<Model>("kid_walk");
            hero[RUN] = Content.Load<Model>("kid_run");
            ...
            SkinningData skinDataIdle = hero[IDLE].Tag as SkinningData;   if (skinDataIdle==null) throw new InvalidOperationException("This model does not contain a SkinningData tag.");
            idleAnimPlayer = new AnimationPlayer(skinDataIdle);
            SkinningData skinDataWalk = hero[WALK].Tag as SkinningData; if (skinDataWalk == null) throw new InvalidOperationException("This model does not contain a SkinningData tag.");
            walkAnimPlayer = new AnimationPlayer(skinDataWalk);
            SkinningData skinDataRun = hero[RUN].Tag as SkinningData; if (skinDataRun == null) throw new InvalidOperationException("This model does not contain a SkinningData tag.");
            runAnimPlayer = new AnimationPlayer(skinDataRun);           // Create an animation player, and start decoding an animation clip.
            ...
            // START AN ANIMATION 
            AnimationClip idleClip = skinDataIdle.AnimationClips["Take 001"];  // "Take 001" <- ASSUMES you exported with default settings (if used scene name it may be that instead)            
            AnimationClip walkClip = skinDataWalk.AnimationClips["Take 001"];
            AnimationClip runClip  = skinDataRun.AnimationClips["Take 001"];
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            // ADJUST_HERO_SIZE and ROTATION:
            mtx_hero_rotate = Matrix.CreateScale(0.26f) * Matrix.CreateFromYawPitchRoll(hero_angle-MathHelper.ToRadians(90), 3.14f, 0);
        ...
        protected override void Draw(GameTime gameTime)
        {
           ...
           // SETUP FOR SHADOW:             
            ...
            Matrix ShadowTransform = Matrix.CreateShadow(new Vector3(litDir.X, litDir.Y, litDir.Z), plane); // matrix to project shadow onto plane:
            ...
            foreach (ModelMesh mesh in hero[IDLE].Meshes) {
                ...
                heroFX.SetShineAmplify(100f);
                foreach (ModelMeshPart part in mesh.MeshParts) {
                    SkinnedEffect oldEffect = (SkinnedEffect)part.Effect;
                    heroFX.alpha = oldEffect.Alpha;
                    if (heroFX.alpha > 0.6f) continue;
                    heroFX.alpha = 0.2f;
                    heroFX.SetDiffuseCol(Color.Blue.ToVector4());
                    heroFX.SetSpecularCol(new Vector3(100f, 100f, 100f));
                    heroFX.SetSpecularPow(256);
                    heroFX.SetBoneTransforms(blendTransforms);
                    heroFX.world = mtx_hero_rotate * Matrix.CreateTranslation(hero_pos);
                    heroFX.SetDrawParams(cam, oldEffect.Texture);
                    ...
                    gpu.Indices = part.IndexBuffer;
                    gpu.DrawIndexedPrimitives(PrimitiveType.TriangleList, part.VertexOffset, part.StartIndex, part.PrimitiveCount);
                    heroFX.SetEmissiveCol(oldEffect.EmissiveColor);
                    heroFX.alpha = oldEffect.Alpha;
                }
                heroFX.SetShineAmplify(1f);
                // DRAW SHADOW
                foreach (ModelMeshPart part in mesh.MeshParts) {
                    SkinnedEffect oldEffect = (SkinnedEffect)part.Effect; if (oldEffect.Alpha < 0.6f) continue; // skip the loop if it's see-thru
                    heroFX.alpha = 0.9f;
                    heroFX.preferPerPixelLighting = false;
                    heroFX.SetDiffuseCol(Color.Black.ToVector4());
                    heroFX.world *= ShadowTransform;
                    heroFX.SetDrawParams(cam, null);
                    gpu.SetVertexBuffer(part.VertexBuffer);
                    gpu.Indices = part.IndexBuffer;
                    gpu.DrawIndexedPrimitives(PrimitiveType.TriangleList, part.VertexOffset, part.StartIndex, part.PrimitiveCount);
                }
